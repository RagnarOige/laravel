...
class TimetableNotification extends Command
{
    ...
    $endOfWeek = now()->addWeek()->endOfWeek();
        ...
        $cachedResponse = Cache::remember($startOfWeek->toDateString(), now()->addHour(), function() use ($startOfWeek, $endOfWeek) {
        ...
        Mail::toconfig('mail.notification.email')
            ->send(new Timetable($items, $startOfWeek->translatedFormat('d. F Y'), $endOfWeek->translatedFormat('d. F Y')));
